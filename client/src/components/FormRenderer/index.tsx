...
export default function FormRenderer({ template, data, onChange, recordId, excelAttachments, onExcelImported, uploadCtx }: FormRendererProps) {
  ...
  const renderField = (field: FieldDefinition) => {
    ...
    if (field.type === 'free_grid') {
      ...
      // 记录侧样品带（sample_band 无 matrix_code = 自引用）：录入时按样品数展开带内行
      const band = ft.sample_band && !ft.sample_band.matrix_code ? ft.sample_band : null;
      const bandRowIdx = band && band.axis === 'row' ? rows.findIndex(r => r.id === band.ref) : -1;
      const sampleCount = Math.max(1, Math.round(Number(gridVal['__sample_count__']) || 1));
      const setSampleCount = (n: number) => handleChange(field.code, { ...gridVal, __sample_count__: String(Math.max(1, Math.min(50, n))) });
      const displayRows: { r: (typeof rows)[number]; ri: number; sample: number | null }[] = [];
      rows.forEach((r, ri) => {
        if (bandRowIdx === ri) { for (let i = 0; i < sampleCount; i++) displayRows.push({ r, ri, sample: i }); }
        else displayRows.push({ r, ri, sample: null });
      });
      // 录入键盘导航：Enter 下移，↑↓←→ 跳格（按显示行 di 定位）
      const gridFocus = (tr: number, tc: number) => {
        const r = Math.max(0, Math.min(displayRows.length - 1, tr)), c = Math.max(0, Math.min(cols.length - 1, tc));
        const el = document.querySelector(`input[data-gp="${field.code}|${r}-${c}"]`) as HTMLInputElement | null;
        if (el) { el.focus(); el.select?.(); }
      };
      const gridKey = (e: React.KeyboardEvent<HTMLInputElement>, di: number, ci: number) => {
        const kk = e.key;
        if (kk === 'Enter' || kk === 'ArrowDown') { e.preventDefault(); gridFocus(di + 1, ci); }
        else if (kk === 'ArrowUp') { e.preventDefault(); gridFocus(di - 1, ci); }
        else if (kk === 'ArrowLeft') { e.preventDefault(); gridFocus(di, ci - 1); }
        else if (kk === 'ArrowRight') { e.preventDefault(); gridFocus(di, ci + 1); }
      };
      return (
        <div key={field.id} style={{ margin: '6px 0' }}>
            ...
            <table style={{ borderCollapse: 'collapse' }}>
              <tbody>
                {displayRows.map((dr, di) => (
                  <tr key={`${dr.r.id}#${dr.sample ?? 'x'}`}>
                    {cols.map((c, ci) => {
                      if (covered.has(`${dr.ri},${ci}`)) return null;
                      const k = `${dr.r.id}::${c.id}`;                          // 模板键（配置：表头/类型/选项/单位…）
                      const vk = dr.sample != null ? `${k}::s${dr.sample}` : k;  // 取值键（带内含样品号）
                      const sp = spans[k];
                      const cspan = Math.min(Math.max(sp?.colspan ?? 1, 1), cols.length - ci);
                      const rspan = dr.sample != null ? 1 : Math.min(Math.max(sp?.rowspan ?? 1, 1), rows.length - dr.ri);
                      ...
                      return (
                        <td key={c.id} colSpan={cspan > 1 ? cspan : undefined} rowSpan={rspan > 1 ? rspan : undefined}
                          ...
                          {ft.cell_options?.[k]?.length ? (
                            <Select size="small" variant="borderless" style={{ minWidth: 76, width: '100%' }} value={gridVal[vk] || undefined}
                              placeholder="选择" allowClear options={ft.cell_options[k].map((o: string) => ({ value: o, label: o }))}
                              onChange={(v) => setCell(vk, (v as string) || '')} />
                          ) : isInput ? (
                            <span style={{ display: 'inline-flex', alignItems: 'center', width: '100%' }}>
                              <Input size="small" variant="borderless" data-gp={`${field.code}|${di}-${ci}`} onKeyDown={(e) => gridKey(e, di, ci)} value={gridVal[vk] ?? ''} onChange={(e) => setCell(vk, e.target.value)} style={{ textAlign: 'center' }} />
          ...
          {band && (
            <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 12, color: '#888' }}>样品数：</span>
              <Button size="small" disabled={sampleCount <= 1} onClick={() => setSampleCount(sampleCount - 1)}>－</Button>
              <b style={{ minWidth: 18, textAlign: 'center' }}>{sampleCount}</b>
              <Button size="small" type="primary" ghost onClick={() => setSampleCount(sampleCount + 1)}>＋ 样品</Button>
            </div>
          )}
        </div>
      );
