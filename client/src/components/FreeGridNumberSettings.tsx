...
export default function FreeGridNumberSettings({ table, cellKeys, onChange }: { table: Table; cellKeys: string[]; onChange: (table: Table) => void }) {
  ...
  return <Popover trigger="click" rootClassName="free-grid-number-settings-popup" onOpenChange={open => { if (open) setScope('selection'); }} title="数字格式与修约" content={<Space direction="vertical" style={{ maxWidth: 'min(360px, 80vw)' }}>
    ...
    <Space wrap><Select aria-label="修约方式" placeholder="多种修约规则" disabled={disabled} style={{ width: 210 }} value={mixedRounding ? undefined : rounding.mode} onChange={mode => setRounding({ mode, digits: rounding.digits ?? 2 })} options={[
      { value: 'none', label: '不修约' }, { value: 'half_up', label: '四舍五入' }, { value: 'half_even', label: '四舍六入五成双' }, { value: 'truncate', label: '直接截尾' },
      { value: 'ceil', label: '向上修约' }, { value: 'floor', label: '向下修约' }, { value: 'multiple_2', label: '修约到偶数' }, { value: 'multiple_5', label: '修约到 5' },
    ]} />{!mixedRounding && !['none', 'multiple_2', 'multiple_5'].includes(rounding.mode) && <InputNumber aria-label="修约位数" disabled={disabled} min={0} max={10} value={rounding.digits ?? 2} onChange={digits => { if (digits != null) setRounding({ ...rounding, digits }); }} />}</Space>
    ...
    <span style={{ fontSize: 12, color: '#888' }}>仅影响本次记录；先修约再格式化。试样区域同一模板格的各试样共享规则。</span>
