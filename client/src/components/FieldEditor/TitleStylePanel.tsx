 ...
 */
import { Card, Input, Space, Popover, Button, Tooltip } from 'antd';
...
function FormatPopover({ value, onChange, title }: { value?: StyleOverride; onChange: (s: StyleOverride | undefined) => void; title: string }) {
  return (
    <Popover trigger="click" placement="bottomLeft" title={title}
      content={<FormatPanel value={value} onChange={onChange} />}>
      ...
      <Space direction="vertical" size={10} style={{ width: '100%' }}>
        <div>
          <div style={{ fontSize: 11, color: '#666', marginBottom: 2, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <FormatPopover title="大标题格式（覆盖默认：标题倍率加粗居中）"
