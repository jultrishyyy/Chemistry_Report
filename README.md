...
│       │   ├── components/EditorToolbar.tsx  # 模板编辑器统一紧凑顶栏（返回/标题/状态/预览/保存/更多）
│       │   ├── components/EditorHistoryControls.tsx # 四类编辑器共用撤回/重做/进入时重置按钮
│       │   ├── hooks/useEditorHistory.ts      # 编辑器级 JSON 快照历史（50 步、连续操作合并）
...
│           │   ├── index.tsx
│           │   ├── FieldPropsPanel.tsx # 字段属性面板（report binding / 字段级格式；semantic_role 不再手选，由「溯源信息」预设固化、此处只读显示）。**字段【类别】+【必填】提到 Tab 区上方【常驻行】(`categoryRow`)——切类别是跨类型高频操作。**已彻底取消独立的「基础」Tab**，原「基础」内容按类型分流：① 简单值(文本/数字/日期/选择)+时间范围→ 单位/行数/说明/**选项(ChoiceEditor)** 全并进「字段值」Tab（选择字段的选项/单选多选/允许自定义与「默认值」同页），只剩 字段名/字段值/格式 三页；② 设备(device_ref)/图片表(report_photo_table)/图片组(report_image_gallery)→ 标签/显示标题/备注/说明 合成 `basicMetaBlock` 注入各自「类型配置」Tab 顶部，只剩 类型配置/格式；③ 间隔(spacer)→ 标签/说明对空白块无意义，直接丢弃，只剩 类型配置；④ 表格类(`hasLabelNoteTabs`)→ 标签/备注在独立 Tab、基础本就空白。非图片表格（data_matrix / free_grid自由表格 / report_result/equipment/sample/conclusion_table，见 `LABEL_NOTE_TAB_TYPES`）统一 4 Tab：类型配置 / 格式 / 标签 / 备注（自由表格也纳入，编辑卡与试验数据表格一致；且在 `NO_FORMAT_BINDING` 内 → 无「取值绑定」Tab，格子级绑定/格式在「类型配置」画布逐格设）。**Tab 命名统一：原「版式」一律改叫「格式」**（简单值的 layoutTab、表格/图的 figureLayoutTab 都改；二者与另一处 formatAllowed 的「格式」Tab 互斥出现、不会同屏两个「格式」）。**「字段名」与「表格标题」彻底解耦（`table_title` 独立字段）**：`field.label`＝【仅编辑器显示名】(字段列表/选择器用)、**不进渲染**——表格类在 Tab 上方常驻行编辑（`hasLabelNoteTabs` 才显示"字段名"输入）。表格标题存独立字段 **`field.table_title`**，「标签」Tab 绑定它，**默认留空＝不显示**、填了才显示（所见即所得）。渲染端只认 `table_title`：`embedDataMatrixTypst`(两处) 与 `wrapFigure`(仅 `TABLE_TITLE_TYPES`＝free_grid/report_result/equipment/sample/conclusion) 都取 `table_title`、非空才出标题；图片表/组(report_photo_table/image_gallery)仍用 `label`+`hide_label`(缺省隐藏)。**注意：存量表格(table_title 未设)现在一律不出标题**（即使 label 有值），要标题就到「标签」Tab 填。这修了"标签框显示字段默认名(如『自由表格』)但渲染不出/或渲染出不想要的默认名"的问题。「备注」Tab＝纯文本框，固定显示在表格【下方】（去掉「表格上方/下方」切换，缺省 caption_position=below）。「格式」Tab 放整表文字（字体/字号/表头·内容加粗）+ 对齐间距——样品信息表/检测结论表还多一个「内容对齐」(写 `table_style.cell_align`，渲染端 renderSampleTable/ConclusionTable 生效)。检测结论表「类型配置」的列改为逐列 开关+改列名(column_labels)+列宽，与样品信息表同款 UI。**字体/字号控件【所见即所得】：未设本级时直接显示【当前实际生效值】而非"跟随默认/继承"占位——`FormatPanel` 收 `inheritedFont`/`inheritedSize`、表格 `table_style` 控件取 本级 ＞ 整表 ＞ 文档默认。文档默认＝`theme_config.font || 'Songti SC'`(宋体) / `body_size ?? 10`pt（与 record-theme `_cfg-get("font","Songti SC")`、DocumentStylePanel 同口径）；`FieldPropsPanel` 与分区头(index.tsx `GroupCard`)都透传 docFont/docSize。字号/行距/对齐等「继承」文案统一改「默认」。**「加粗」控件同理去掉"跟随"**：只留 加粗/正常 两项、直接显示当前生效粗细——`FormatPanel` 收 `inheritedBold`(标题/字段名默认加粗→传 true，正文/备注默认正常→false)，`FieldPropsPanel` 的字段名加粗控件按 `docLabelBold`(=`theme_config.label_weight`≠regular) 显示；TitleStylePanel 大标题传 boldDefault。**「距离」旋钮（标签/备注与表格的距离、分区标题与内容间距等）同理**：未设时直接显示【当前默认值】(pt)——渲染端缺省跟随文档「字段间距」(line_gap)，`docGapPt`＝line_gap 换算 pt（em×正文字号）；不再显示"默认"占位。常驻行的字段说明提示、「必填」右侧的"可留空"文案已改为悬浮提示（`QuestionCircleOutlined`）。表格「标签/备注」Tab 文案精简：Alert 只留标题（表格标签/表格备注）、去掉 Form.Item 里的「（字体/字号/加粗/斜体/颜色）」「（留空＝不显示标题）」等括注。自由表格/试验数据表格（free_grid / data_matrix）的「格式」Tab 另给一组「表头文字 / 内容文字」控件＝表头与内容【分开】设 加粗+字体+字号**（存 `table_style.{header_bold,body_bold,header_font,header_font_size,body_font,body_font_size}`；渲染端 `cellStylePair`→`cellTextStyled` 逐格生效：缺省仍表头加粗、内容常规，与历史输出一致；data_matrix 表头由 `*…*` markup 改等价 `#text(weight:"bold")`，仿宋等无粗体字体多出 faux 描边，与其它报告表一致）。**非图非表字段（简单值/时间范围/设备等 `#field` 渲染的字段）的「格式」Tab 增「字段名↔字段值 距离」旋钮＝字段级 `field.label_width` 覆盖（跟随分区/紧贴(none)/拉开(如 8em)），优先级 字段 ＞ 分区 group.label_width ＞ 文档 theme_config.label_width ＞ none；渲染端 `generateFieldBlock` 的 `effLabelWidth` 已按此优先级取值传给主题 `#field(label_width:)`（仅竖排字段有效）**。**报告模式（`caps.valueBinding`，report-cover/report-project）编辑卡精简（2026-07-16）**：值来自绑定映射、不录入，故**隐藏所有录入设置**——「必填」开关、简单值的「单位/行数」、选择字段「选项(ChoiceEditor)」、「录入提示(说明)」、设备/图片的「字段说明」全部不显示（只留 字段名/取值绑定/格式）。**格式 Tab 去掉"默认/跟随"占位**：「对齐」传 `FormatPanel alignNoInherit`＝去掉「默认」项、未设直接显示实际生效值「左」；「字段名↔字段值 距离」去掉「跟随分区」项、未设显示实际默认「紧凑」（记录模式仍保留继承项，不变）
...
│           ├── external-report-delivery.ts # 接口1.4出站 SOAP 客户端：submitReportToDiGui(sysNumber, pdfBase64, jobNo) 按业务系统 WSDL 方法 AcceptReportFromDiGui(sysNumber, file, jobNo) 构 SOAP 1.1/1.2 信封回传报告 PDF（方法名/三个参数名/namespace/action/endpoint 全可配 config/report-delivery.json 或 DELIVERY_*，空 endpoint=mock=不真发）。⚠️ 少 jobNo(操作人登录账号工号) 对方拒收；参数名须与对方 WSDL 元素名一致
...
│   ├── binding-integrity.ts   # 报告映射引用完整性校验（报告 binding vs 关联原始记录字段集）；含 free_grid cell_bindings（record_free_cell/_sample）+ source_field 样品带校验
│   ├── report-inherit.ts      # 项目模板继承原始记录（§10.5 任务二）：buildProjectGroupsFromRecord——记录字段→报告 groups 一次性快照 + 自动配映射（简单字段=record_field 恒等绑定 / free_grid=结构镜像+逐格绑定+source_field 动态试样带 / data_matrix 样品表=报告结果表试样带一键成表[试样编号+每参数一列+band，行高 cell_inset_y 对齐记录 5pt、label_gap 12pt]、统计表进手动 / 语义角色=record_meta / spacer 原样；image/device_ref 进「需手动」清单）。**返回 theme_config=DEFAULT_PROJECT_THEME_CONFIG**（取自「弯曲强度&弯曲模量」模板：FangSong_GB2312 / body_size 10.5 / line_gap 1.9 / paragraph_gap 1.6 / section_style left-bold / margin wide），拉取/新建时写进 layout_options.theme_config 统一文档样式
...
**测试设备常用项（2026-08-03）**：`device_ref` 保持独立业务类型，不复用普通 `select`。原始记录模板可配置 `device_ref_config.preset_asset_codes`（常用设备）、`selection_mode`（单台/多台）和 `allow_library_search`（是否允许从完整设备库选其他设备）；常用项仅作快捷候选、不自动写值。录入端“常用设备 + 从设备库选择其他设备”最终统一保存 `asset_code[]`，兼容存量记录和报告汇集。进入待审核状态的三个写入口均在服务端校验必填、单选数量、限定范围、设备存在性、停用/报废/不合格/超期状态；草稿仍可保存。报告生成遇设备缺溯源/到期日期时保留设备行并以空值渲染，同时返回警告，不再把整台设备从报告表漏掉。
...
|---|---|---|---|
| **委托单数据源（接口 1.1 PushOrderInfos）** | **接收端点已实现**：`POST /api/external/orders`（`routes/external.ts`）收外部推送 → `services/external-orders.ts` `parseOrderInfos`（全字段：订单 meta + 样品 BarCode 作 id + 材料分单全字段）→ upsert `work_orders`。`fetchExternalOrders()` 拉取接缝仍返回空数组（占位）。演示期可 POST `EXAMPLE_PUSH_ORDER` 或界面「新建订单」 | 外部按 1.1 契约 POST 到 `/api/external/orders` 即可（推送模型，无需改我方）；若为拉取模型则换 `fetchExternalOrders()` 函数体 | **录入端接口上下文已展示（2026-08）**：订单详情样品列明确显示 `SampleSortNo` 样品编号、`BarCode` 条码和型号；每个外部测试项目下可折叠查看 TaskList 的主机厂/标准/测试方法与条件/制样/限值/负责人/日期/备注/描述/分单人等 15 个字段（空值显示 `—`）。进入具体数据录入后，左侧表单顶部显示同一套“当前样品 × 项目”折叠卡片。共用组件 `pages/Lab/OrderProjectInfo.tsx`，字段映射详见 `待实现内容.md` 第 1 节 |
...
| 改自由表格 / 统一网格（`free_grid`，F0） | 编辑器 `client/components/FieldEditor/MatrixEditor/FreeGridCanvas.tsx`（加行列 / Shift 框选合并·拆分 / 标表头·录入格 / 填固定文字；**选中单格或多格可直接设置字体、字号、粗体、斜体、颜色、对齐并清除单元格格式，写入 `free_table.cell_styles`；优先级＝单元格格式 ＞ 表头/内容默认格式 ＞ 整表/文档默认。格式随行列拖动，转置时按坐标转置；合并保留主格格式并清理被覆盖格。**双击·右键格子弹「单元格编辑卡」——顶部「角色」下拉 表头/录入/公式 互斥切换(`setRole`，公式仅单格→开公式面板；固定文字不再作新选项、仅存量保留显示)，下接 数据类型(文字/数字/选择)/选项/单位(固定·可选)/数字格式/列宽行高**；界面提示尽量收进问号悬浮(`zIndex:2100` 压过编辑卡)。「选择」格＝录入格，画布与图例均显示"录入·选择"。**可选单位**(cell_unit_options)未选/模板期渲染回落到【首个选项】占位显示(`renderFreeGridTypst` 的 `cellUnit`)——修"可选单位不显示"）+ `FieldPropsPanel.tsx` 分发（**字段属性 5 Tab 与试验数据表格一致：基础/类型配置/版式/标签/备注**——`free_grid` 已并入 `LABEL_NOTE_TAB_TYPES` + `NO_FORMAT_BINDING`，标签/备注为纯文本框，详见上「代码结构总图」FieldPropsPanel 条）+ `field-types.ts` 注册（「自由表格」，`editors:['record','report-project']`，**两处 switch** `createFieldForCategory`/`rebuildFieldForCategoryInner`）+ `FormRenderer/index.tsx` 录入分支（录入格填值存 `raw_data[code]` 的 `${rowId}::${colId}`，并按 `cell_styles` 显示）。渲染 `shared/typst-generator.ts` `renderFreeGridTypst`（纯网格·无强制表头行·`header_cells` 默认加粗·`spans` 自由合并·`cell_styles` 逐格覆盖·`dataOverride` 填录入值）；`generateFieldBlock` 发 `__FREE_GRID__` 锚点 → `generateTypstWithData` 按锚点替换成带录入值版本（与 `data_matrix` 同机制）；`flattenDataForDisplay` 把 free_grid 剔出 `#let data`。数据模型 `shared/types.ts`：`FieldType 'free_grid'` + `free_table.{cells,spans,header_cells,input_cells,cell_bindings,cell_styles}`。**F1（报告侧，已实现）**：`editors` 加 `report-project`；每格 `cell_bindings` 绑原始记录（`renderFreeGridTypst` 加 `ctx` 按 `resolveBinding` 取值，优先级 绑定>录入值>固定文字；`injectReportFieldsIntoTypst` 按 `__FREE_GRID__` 锚点分发；`FreeGridCanvas` 传 `linkedRecord` 时出「绑定原始记录」按钮 + `BindingPickerModal`）。**F2（报告侧样品带，已实现）**：`free_table.sample_band{axis,matrix_code,ref}` + `expandFreeGridBand`（渲染前按记录矩阵实际样品数展开带内行/列，`record_cell_sample`/`record_sample_label`/`record_sample_index` 逐样品落地，镜像 `expandResultTableBand`）；编辑器「样品来源矩阵 + 设为样品带·行/列」。**F3（每格公式，已实现）**：`free_table.cell_formulas`（复用 `Formula`+`formula-engine.execute`，`sources` 引用其它格 `${rowId}::${colId}`）；`renderFreeGridTypst` 两遍算值（基础值→公式）；`expandFreeGridBand` 带内公式逐样品、带外聚合展开到全部样品；编辑器「公式」按钮点格选来源。**替代统计/汇总行列 + 覆盖需求 #6 限值自动判定。** 设计见 `编辑器改进方案.md §20.5` / `报告映射方案.md §10.7`。**F1b·项目模板从原始记录拉取（✅ 2026-07-15，§10.5 任务二）**：`ProjectEditor` 顶栏「从原始记录拉取」→ `shared/report-inherit.ts` 的 `buildProjectGroupsFromRecord(linkedRecord)` 把记录字段【整体替换】成报告 groups 并自动配映射，弹映射小结（自动 N / 需手动 M）。**free_grid 从记录 free_grid 拉取时**：结构镜像（表头/固定文字/合并/宽高/单位/公式全拷）、记录「录入格」`input_cells`→报告 `cell_bindings`（带内 `record_free_cell_sample`、带外 `record_free_cell`，两新绑定源读 `record_raw_data[记录code][rowId::colId(::s{i})]`）、记录【自引用样品带】→报告 `sample_bands` 加 `source_field`(记录 free_grid code)+`source_band_id`；`expandFreeGridBand` 遇 `source_field` 带时样品数从记录该 free_grid 的 `__sample_count__::bandId` 派生（`deriveFreeGridSampleCount`，缺则按 `::s{i}` 键推断）→**录入期增减试样，报告渲染自动展开对应份数**（动态试样闭环，不碰表头文本）。`FreeGridCanvas` 报告侧把 `source_field` 带识别为报告样品带（不当自引用带、带角标可移除）；`binding-integrity` 校验 `record_free_cell(_sample)`+`source_field` 引用完整性。⏳ 录入端公式实时预览；记录侧录入端样品带（记录侧已实现，报告侧多带留后） |
...
**F2c·自由表格参数轴受控（2026-07-21）**：报告自由表格用普通“+ 行 / + 列”扩充版式，不再提供专用“+ 参数行 / + 参数列”。用户在既有试样数据区域内选中格子后点“按每个试样取值”，该格才可在“映射”中选择逐试样参数或试样序号；按钮旁问号说明其展开行为和适用范围。未标记格只能选择单值来源。原始记录试样按列展开时标记形成参数行，按行展开时形成参数列，并用青绿色边线区分；映射保存后标记由 `record_free_cell_sample` / `record_sample_index` 自动推导，清除映射即取消标记。转置后入口和颜色跟随 `sample_bands.axis` 自动换轴；`binding-integrity` 会拒绝落在对应 `source_field` 样品带之外的逐试样绑定，避免模板轴长度与实际试样数错位。**从原始记录拉取时不能只按样品带行/列粗略判定**：横跨多个样品成员的合并格以及“结论/判定/备注/说明/汇总”等共享语义格必须生成普通 `record_free_cell`，只有真正的试样参数格才生成 `record_free_cell_sample`；编辑器同样禁止在共享格开启按试样取值。

**编辑器历史与重置（2026-07-22）**：原始记录模板、报告首页模板、报告项目模板和生成报告编辑器统一使用 `useEditorHistory` + `EditorHistoryControls` 提供“撤回 / 重做 / 重置到进入时状态”。历史按完整模板或 `content_doc` JSON 快照记录，最多 50 步，连续文字输入、颜色滑动和宽高拖动在 400ms 内合并为一步；字段、表格、映射、公式、格式与文档版式严格进入同一条操作链。异步加载完成后的真实模板是基线；项目模板创建时若自动从关联原始记录拉取，则自动拉取完成后的内容才是基线。保存只落库，不改变本次进入页面时的重置目标；重置要求二次确认，且重置本身仍可撤回。只读历史版本和审核中页面隐藏历史按钮。

表格字段不再维护第二套局部撤回/重做栈，避免全局与局部历史顺序冲突。`FieldPropsPanel` 仍保留“重置默认”，它只替换当前表格类型的内部结构配置（会清除行列、合并、公式和格子绑定），保留字段 id/code、字段名、表格标签、备注与外层格式；该操作作为一步进入编辑器全局历史，可从顶栏撤回。

**自由表格任意插入与拖拽排序（2026-07-21）**：`FreeGridCanvas` 在表格顶部和左侧增加编辑器专用列/行拖拽把手（点击把手选中整列/整行，拖到目标把手前半/后半决定放置位置），原始记录与项目报告共用；局部合并格跨越的连续行/列及样品带成员跨区移动时作为整体，同一结构块内部则允许单行/单列重新排序，执行前校验合并格仍连续、主格仍在左上且试样带不被拆开，不合法时给出明确提示；覆盖整条轴的合并格不阻塞该轴排序。拖放源同时写入浏览器 `dataTransfer`，避免 React 临时状态未刷新导致松手无效。单元格 id 不变，因此文字、绑定、公式引用和尺寸随轴一起移动，移动后自动选中新的位置。顶部“+ 行 / + 列”在有选区时分别插到选区下方/右侧，无选区时追加到表格末尾，选区栏不再重复显示四个方向插入按钮。插入点位于合并格内部时自动扩展跨度并避免给被覆盖格写隐藏元数据，位于样品带成员之间时新轴自动加入该带，带前/带后插入则保持普通轴。两类编辑器共用相同的结构操作布局；“表头 / 录入 / 公式”是原始记录录入语义，只在原始记录模板显示，报告项目模板隐藏这些按钮并保留来源绑定。所有操作进入统一撤回/重做历史。

**自由表格合并/拆分选区约定（2026-07-21）**：合并后选区保持完整合并矩形；拆分单个或多个真实 `spans` 后，选区自动覆盖拆分前的完整范围，因此可直接再次点击“合并”，不需要重新框选。拆分只删除 `free_table.spans`，不会新增行列或空白格。合并选区允许包含已映射格：无来源直接合并，唯一来源（含多个相同来源）自动迁移到左上主格，多个不同来源弹窗由用户选择保留哪一个；被覆盖格中的其他映射、公式和逐格格式同步清除，避免再次拆分时隐藏数据重新出现。

**自由表格拖动框选约定（2026-07-22）**：原始记录与报告项目模板共用锚点式框选。鼠标按下的逻辑格固定为锚点，移动时按锚点与当前格实时重算矩形，不累计历史经过区域；因此支持从任意位置向左、向右、向上、向下或斜向拖动，也支持回拖收缩。命中真实合并格或逐试样视觉合并区时仍完整纳入对应逻辑格。

**自由表格表头开关约定（2026-07-21）**：原始记录与报告项目编辑器共用一个紧凑的“表头”开关，不再分别显示“设为表头 / 改为普通格”文字按钮。报告模式切换表头只改变灰色表头语义和默认文字格式、保留已有映射；原始记录模式仍保持表头与录入/公式角色互斥。

**自由表格选区字体显示约定（2026-07-21）**：字体/字号控件始终显示选区当前实际生效值，按“单元格覆盖 ＞ 表头/内容默认 ＞ 整表默认 ＞ 文档默认”解析，不显示“默认字体/默认字号”；多选格的实际值不一致时才显示“多种”。仅查看继承值不会写入 `cell_styles`，用户修改后才产生逐格覆盖。

表格画布中的固定中文、报告映射状态、逐试样状态和操作提示都必须遵循用户显式设置的逐格字体、字号、字重、斜体及颜色；组件内部的状态默认色和层级字号只在该格没有显式覆盖时生效，不能造成数字可改而中文不可改的假象。

加粗优先使用字体自身的真实粗体字形；仿宋、楷体、黑体等仅提供常规字重的字体在 Typst/PDF 中使用轻量 `0.015em` 描边补偿，避免旧 `0.03em` 描边产生明显黑色覆盖感。编辑画布仍使用浏览器原生 `font-weight`，两端都不通过改变文字颜色模拟加粗。

**自由表格内容与单位映射约定（2026-07-21）**：从原始记录拉取自由表格时，录入格继续写 `cell_bindings`，表头文字新增 `record_free_template_cell` 映射；公式格不读取不存在的 raw 值，而是生成 `record_free_formula_cell` / 样品带内 `record_free_formula_cell_sample`，报告生成时用原始记录数据执行原公式。报告副本删除 `cell_formulas`，避免同时维护“本地公式”和“原公式结果绑定”。原始记录中任意格配置的固定/可选单位都单独拉取到 `cell_unit_bindings`（`record_free_cell_unit` / 样品带内 `record_free_cell_unit_sample`），不再把单位能力限制在表头。报告渲染按“内容（单位）”组合输出，内容来源和单位来源可以分别修改、清除；表头仅控制灰底及默认文字格式，不影响两类映射。

继承自由表格用 `source_field_code` 记录来源，后续“映射”默认打开紧凑双来源选择器：上方固定显示“内容来源 / 单位来源”两个槽位；弹窗标题随当前槽位明确显示“选择内容来源 / 选择单位来源”，不使用“这个位置要显示什么”等含混文案。内容固定分成“原始记录表格 / 原始记录字段”两个 Tab；表格 Tab 可在关联记录的多个自由表格间切换，录入格、模板文字和公式结果格均可选择，字段 Tab 展示普通字段和计算字段。单位来源只分为“表格单位 / 字段单位 / 固定单位”三类，不再出现含义不清的“矩阵参数单位”；字段单位使用 `record_field_unit` 读取普通字段模板配置的 `unit`。表格单位页先过滤整张表：没有任何固定单位或单位选项的自由表格不进入选择列表，全部表格均无单位时显示明确空状态；有单位的表内只允许选择确实配置了固定单位或单位选项的格子，并以青色底框和“单位：…”直接标明。逐试样单位仅在逐试样目标格中使用 `record_free_cell_unit_sample`，普通目标格使用单值单位来源，避免脱离样品上下文后解析为占位值。弹窗确认时两类来源通过一次 `free_table` 更新原子提交，禁止拆成连续两次 `onChange`，否则后一次可能基于旧字段快照覆盖前一次；仅编辑单位时不得顺带生成空内容映射。

报告自由表格画布采用渐进披露：格子正文只显示最终内容的短名称，不直接铺开完整来源说明；右上角单一链路图标表达映射状态（蓝＝仅内容、青＝内容+单位、橙＝仅单位），悬浮后显示完整双来源。单选格或单个逻辑合并格后，操作栏下方显示两行来源摘要及各自“修改 / 清除”，多选仍只保留区域级清除，避免表格内信息过载。

**自由表格试样序号约定（2026-07-21）**：记录侧样品带录入格可在单元格编辑卡开启“试样自动序号”（`sample_index_cells`）；录入界面和记录 PDF 按展开顺序直接生成 1、2、3…，报告继承生成 `record_sample_index`，不复制或依赖录入值中的“1”。旧模板未设置显式标记时仍按“序号/编号”表头推断，保持兼容。

**映射选择器展示约定（2026-07-21）**：报告映射、格子绑定、设备来源和图片来源等面向业务用户的选择器只显示字段中文名称，不显示 `service_no`、`field_123`、`select` 等内部 code/type；code 仍作为选项 value 和 `CellBinding` 持久化，不改变历史模板兼容与解析。缺少中文 label 的存量字段统一显示“未命名字段/表格/参数”，不回退暴露内部编码。

**报告自由表格映射交互（2026-07-21）**：报告项目模板中单击格子只改变选区，不直接弹映射窗口；选区栏提供“选择映射 / 清除映射”。选择映射支持普通单格以及单个真实/逐试样视觉合并格（视觉合并虽跨多个物理格仍按一个逻辑格处理），清除映射支持多选且清除多个时二次确认；清除逐试样逻辑格后选区自动收回主格，保证可立即重新选择映射。双击格子保留为快速打开映射的入口。原始记录模板不显示这两个报告映射按钮。

**报告自由表格映射颜色与画布尺寸（2026-07-21）**：报告项目模板按数据状态而非所在参数轴着色：未映射格白色、普通单格映射浅黄色、逐试样条带映射浅青绿色、公式浅紫色、表头灰色；参数行/列和试样展开区只用青色边线表达结构，不给未映射格铺青色背景，避免与逐试样映射混淆。报告侧表格宽度至少铺满编辑卡，列数较多时横向滚动；报告格使用更大的最小列宽和行高。表头格把实际表头文字放在上方并使用更大字号/字重，“已关联映射”缩小放在下方；普通非表头映射格只显示状态，不重复行列来源名称。逐试样格显示“已关联试样数据”，空格显示可点击的“点击添加映射”，完整双来源仍放在悬浮详情和选中格摘要中。

**报告自由表格表头规则（2026-07-21）**：报告侧“表头”只是展示属性：控制灰色背景以及格式面板中的表头加粗/字体/字号，不是与映射互斥的数据角色，因此表头格可以绑定普通单值或逐试样映射，设置/取消表头均保留已有映射和公式。报告项目模板新增行时首列默认设为表头，新增列时首行默认设为表头（顶部“+ 行 / + 列”和尺寸直填均一致）。选区栏只提供一个简化切换“设为表头 / 改为普通格”，不恢复原始记录专用的“录入 / 公式”角色按钮。
...
| 改报告模板编辑器（首页/项目）的公共外壳 | `client/src/pages/ReportTemplate/useReportTemplateEditor.ts`——CoverEditor + ProjectEditor **共享** hook：加载（只读历史版本 / 草稿续编 / `dedupeTemplateIdentity` 自愈）、保存（PUT report-templates + 409 冲突 + 草稿乐观锁 + `binding_warnings`）、未保存守卫、版本元信息（`versionMeta`，供顶栏 `TemplateVersionPanel`）、编辑器⇄PDF 跳转、示例数据开关。页面特定的部分（首页页眉页脚 UI、项目关联记录+binding 校验、各自预览 typst）留在各页，项目用 `onLoaded` 回调按 `linked_record_template_id` 拉关联记录 | 顶部 UI 统一使用 `client/src/components/EditorToolbar.tsx`：图标返回、模板标题、紧凑版本操作、示例/空白分段控件、单一主保存按钮；低频操作进“更多”菜单。原始记录编辑器和旧字段映射编辑器也使用同一外壳。record 编辑器的数据加载/保存仍因 POST 建模板、受控登记和试录等差异保持独立实现。 |
...
- **faux-bold（仿宋/黑体/楷体等无粗体字体的加粗）**：`FandolFang` / 仿宋 `FangSong(_GB2312)` / 黑体 `SimHei` / 楷体 `KaiTi` 都只有 Regular 字重，Typst 对缺字重字体**不合成粗体**，故 `weight:"bold"` 无反应（页眉「检测报告」、字段标签加粗都看不出）。主题 `_nobold-font()` 与生成器 `isNoBoldFont`（命中 `/Fandol|FangSong|SimHei|KaiTi/i`）改用**描边 `stroke: 0.03em`** 模拟粗体。有真粗体的字体（`Songti SC` 等）仍走 `weight`，行为不变。新增单字重字体时记得加进这两处判断。**faux-bold 覆盖面（2026-06 补全）**：早期只有 `#field` 标签/值（经主题 `_bold`）和 `styleSetRules` 走 faux-bold；**分区标题、文档大标题、子分区标题、表格/多列字段标签、签名标签、矩阵表标题、检测结论表标题**等"`#text` 直接加粗"处当时**漏了**（仿宋下这些都不加粗）。现统一：① 生成器 `fauxBoldStroke(font)` 给所有 `#text(weight:700)` 直发处补描边；② 标题专用 `titleTextArgs(style, defaultSize)`（weight+描边+字号+字体/斜体/颜色）供顶级/子分区标题共用；③ 主题 `#section` 默认标题改走 `_bold(...)`、有 `title_args` 时直接套生成器算好的参数；④ 主题文档大标题（`record-theme` 标题块）改 `_bold(text(..title_args)[title], on: weight 是否加粗)`；⑤ **全局 `#show strong`**：主题对 `*粗*` markup / `#strong`（表头 `[*…*]`、富文本 `**粗**`）补 `text(stroke)`（仅 `_nobold-font` 命中时，保留 weight 故西文真粗体不丢）——一处覆盖所有表头/富文本加粗。**⚠️ 表格单元格例外（2026-07-16）**：`tableCellBold`/`cellTextStyled`（结果表 / 自由表格 / 设备·样品·结论表的表头加粗）在无粗体文档字体下**只发 `stroke: 0.03em`、不再发 `weight:"bold"`**——因为拉丁字母/数字会回退到 **Arial**（`font: ("Arial", "仿宋…")`，Arial 有真粗体），若同时给 `weight:"bold"` 会「Arial 真粗体 + 描边」双重加粗，比同格中文重、整行粗细不一（用户反馈"表头字母数字过粗带动整行"）。改为仅描边后中西文粗细一致。⚠️ 这与 `#show strong`（`*…*`/`#strong` 保留 weight、西文走真粗体）的取舍**相反**——表格表头走 `#text` 直发路径、按"中西一致"处理；`*…*` markup 仍保留真粗体。任何新增"`#text(weight:700)` 直接加粗"代码若含中西混排且要求粗细一致，参照此法（只描边）。字体下拉（`DocumentStylePanel`/`FormatPanel`）已加「仿宋（FandolFang·免费）」(值 `FandolFang R`)；本机预览需装该字体（已随 TeX Live 提供，复制到 `~/Library/Fonts`），生产环境用 `仿宋_GB2312`。
...
报告 `CellBinding`（`record_field`/`record_cell`/`record_summary`）用字符串编码指向关联原始记录字段。原始记录改名/删了字段编码后，`resolveBinding` 取不到会静默落到 `'—'`，那格变空且无告警。校验器 `shared/binding-integrity.ts` 的 `validateReportBindings(reportGroups, recordGroups)` 收集报告所有 binding 比对原始记录字段集，返回失效项。已接入：项目报告编辑器（`ProjectEditor.tsx`）显示紧凑可展开警告、`GET /api/report-templates/:id/binding-check`、PUT/approve 响应带 `binding_warnings`。客户端与服务端读取关联记录字段必须同口径：**生效版本优先，没有生效版本时回退最新草稿/待审/退回版本**；否则仅有草稿的原始记录会在保存项目模板时被整表误报失效。**默认只告警不阻断保存**（合规上保存草稿不该被卡）。新增报告 binding 来源时，记得在校验器里加对应分支。

**项目模板自动继承约定（2026-07-22）**：新建项目模板勾选“从关联原始记录拉取并自动配置映射”时，`List.tsx` 不在创建页提前抓取记录快照，只在新模板 `layout_options.pending_record_inherit` 写一次性意图及创建时选中的 `record_template_id/name`。进入 `ProjectEditor` 后，先按该模板自身的 `linked_record_template_id` 加载明确关联记录，并再次核对响应 ID 与创建选择 ID，两者一致才调用 `buildProjectGroupsFromRecord`；生成结果必须立即用同一份关联记录执行 `validateReportBindings` 且得到零错误，才允许整体写入并移除一次性标记、记录 `inherited_record_template_id/version`，否则停止且不覆盖原内容。因此首次自动生成不能带失效映射，后续告警只反映来源字段改名/删除或用户手动改坏绑定。关联记录异步请求带序号，旧请求不得覆盖新模板来源；新建弹窗每次打开/取消均清空上次表单值，避免沿用旧关联。未保存离开仍走统一未保存守卫。项目编辑器明确显示“模板操作”下拉，提供手动重新生成和“重置为空白模板”；空白重置清字段、项目名和结论，但保留模板实体及其原始记录关联。
...
- **删除（归档）需审批（migration 023）**：任何人删除模板都要先「申请删除」（原因可选），由审核员批准后才归档生效；申请人≠批准人、驳回必填备注，与版本审核同一合规约束。申请期间状态列显示「删除待审」，申请人可撤销。直接 `DELETE /:id` 已停用（403）。归档仍是软删：版本历史 + 录入数据完整保留、可恢复。**列表流程按钮固定为「提交 / 撤回 / 审核 / 删除」四个位置**：版本待审时提交和删除禁用，撤回/审核处理版本；删除待审时提交和删除仍保留原位但禁用，撤回/审核改为处理删除申请；无权限或当前状态不适用时按钮只灰显并通过 Tooltip 说明原因，不再随状态增删、跳位。
...
- 服务端强制：**原始记录模板全表名称唯一；报告模板【同 kind 内】名称唯一**（cover/cover_page/project 各自命名空间，同名可跨 kind）——只算**未归档**（`archived_at IS NULL`），比较用 `btrim(name)`（去首尾空格）。
- 落点：`record-templates.ts` `recordNameTaken()` / `report-templates.ts` `reportNameTaken()`，三处入口都校验——**创建 `POST /`、改名 `PUT /:id`、派生 `POST /:id/fork`**；占用返回 **409 `{ error, code:'duplicate_name' }`**。
- 客户端两个模板编辑器（`RecordTemplate/Editor` + `useReportTemplateEditor`）按 `code==='duplicate_name'` 区分：同名→普通 `message.error`，**不**弹「保存冲突/刷新」并发锁 Modal（那是草稿乐观锁的 409）。
- **不建 DB 唯一约束**——现网可能有历史同名数据，加约束会致迁移/部署失败；应用层只拦新的同名，对存量安全。
...
| POST | `/api/external/requisitions/:id/deliver` | **接口 1.4 报告 PDF 回传（出站，**SOAP**）：取已生成报告 `final_typst`→编译 PDF→Base64→`services/external-report-delivery.ts` `submitReportToDiGui` 构 **SOAP 1.1/1.2 信封**调业务系统 WCF 方法 **`AcceptReportFromDiGui(sysNumber, file, jobNo)`**（sysNumber=SysNumber、file=PDF Base64、jobNo=操作人登录账号工号 `X-User-Job` 头；**少 jobNo 对方拒收**）→解析 `AcceptReportFromDiGuiResult`/SOAP Fault→更新 `delivery_status/delivered_at/delivery_error`，并置 `reports.external_status='submitted_external'`+`stale=false`。**送审成功即视为该报告退回修改已处理：关闭其未关闭的 `scope=report` 工单 + 清取号单 `stale`→报告回到终态**（再次退回前送审按钮不可点击）。未生成报告则 400；**该报告有未关闭 `data_entry` 工单（实验室数据退回中）则 409**（须等数据重审通过+重生成）。配置 `config/report-delivery.json`+`.local.json`+`DELIVERY_*`（`soap_endpoint`/`method`/`param_id_name`/`param_file_name`/`param_jobno_name`/`soap_action`，空 endpoint=mock=不真发；以业务系统 WSDL `http://172.18.0.97:8003/Lab/Chemistry` 为准）。⚠️ 参数名(sysNumber/file/jobNo)须与对方 WSDL 元素名一致，否则对方收到调用但取不到值 |
